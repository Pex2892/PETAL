...
function treeBoxes(urlService, jsonData)
{
	...
	var margin = {
        ...
        width = $(".divPrintPlot").width() - margin.right - margin.left,
        height = $(".divPrintPlot").height() - margin.top - margin.bottom;

	var rectNode = { width : 120, height : 20, textMargin : 5 },
		tooltip = { width : 170, height : 40, textMargin : 5 };
	var i = 0,
		duration = 750,
		root;
	...
	function drawTree(jsonData)
	{
		...
		var maxTreeWidth = breadthFirstTraversal(tree.nodes(root), function(currentLevel) {
			...
			currentLevel.forEach(function(node) {
				if (node.type == 'type1')
					//node.color = blue;
				    node.color = palette.color_5;
				if (node.type == 'type2')
					node.color = green;
                    //node.color = palette.color_0;
		...
		baseSvg = d3.select('.divGraph').append('svg')
		...
		getMouseWheelEvent();
		d3.select('.divGraph').select('svg').on(mouseWheelName, null);
		d3.select('.divGraph').select('svg').on('dblclick.zoom', null);
	...
	// 2) ******************* Update the links *******************
		...
		// Enter any new links at the parent's previous position.
			...
			linkTooltip.enter().append('text')
                ...
                .text(function(d) {return "Relation: " + d.target.link.name;})
                .append("tspan")
                .attr('x', function(d) { return (d.target.y + rectNode.width - d.source.y) / 2 + d.source.y + tooltip.textMargin; })
                .attr('dy', '1.5em')
                .text(function(d) {return "Subtype: " + d.target.link.nodeName;})
                .append("tspan")
                .attr('x', function(d) { return (d.target.y + rectNode.width - d.source.y) / 2 + d.source.y + tooltip.textMargin; })
                .attr('dy', '1.5em')
                .text(function(d) {return "From: " + d.target.link.from;});
	...
	function removeMouseEvents() {
		...
		mousedown = d3.select('.divGraph').select('svg').on('mousedown.zoom');
		d3.select('.divGraph').select('svg').on("mousedown.zoom", null);
	}

	function reactivateMouseEvents() {
		// Reactivate the drag and zoom behaviors
		d3.select('.divGraph').select('svg').on('mousedown.zoom', mousedown);
	}

	// Name of the event depends of the browser
	function getMouseWheelEvent() {
		if (d3.select('.divGraph').select('svg').on('wheel.zoom'))
		{
			mouseWheelName = 'wheel.zoom';
			return d3.select('.divGraph').select('svg').on('wheel.zoom');
		}
		if (d3.select('.divGraph').select('svg').on('mousewheel.zoom') != null)
		{
			mouseWheelName = 'mousewheel.zoom';
			return d3.select('.divGraph').select('svg').on('mousewheel.zoom');
		}
		if (d3.select('.divGraph').select('svg').on('DOMMouseScroll.zoom'))
		{
			mouseWheelName = 'DOMMouseScroll.zoom';
			return d3.select('.divGraph').select('svg').on('DOMMouseScroll.zoom');
		}
	}
